...
#define OPENURL		""
#define URL_FIX		L"http://static.missevan.com/app/win/"	//结尾有/


#define UPDATE_INFO_FILENAME	L"update.json"

static wchar_t *sz_jmp_filename[] = {
	L"BuildUpdate.exe", L"TaLzma.exe", UPDATE_INFO_FILENAME
};
...
class App {
public:
	App(LPCTSTR path, LPCTSTR version)
		: m_path(path)
	{
	...
	size_t FileMD5(uint8 *data, size_t length, std::string& file_md5)
	{
		...
		if (memcmp(header->magic, "TALZMA", 5) == 0)
		{
			if (header->compresssize + sizeof(TA_LZMA_HEADER) <= length)
			{
				...
				if (m_original_data)
				{
					int status = LzmaUncompress(m_original_data, &osize, data + sizeof(TA_LZMA_HEADER), (size_t *)&csize, header->props, propsize);
					if (status == SZ_OK)
					{
